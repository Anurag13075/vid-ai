...
import { HeroGrid } from "@/components/motion/hero-grid";
...
function Landing() {
  return (
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--ink)] overflow-x-hidden landing-mesh">
      <Nav />
      <Hero />
      <StatsBar />
      <MarqueeBar />
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-white/95 backdrop-blur-xl border-b border-black/8 shadow-[0_2px_20px_-4px_rgba(0,0,0,0.12)]"
          : "bg-white/70 backdrop-blur-md border-b border-black/6"
      }`}
    >
        ...
        <nav className="hidden md:flex items-center gap-1">
          {[
            ...
            <a
              key={href}
              href={href}
              className="px-3 py-1.5 text-[13.5px] font-medium rounded-lg transition-all duration-200 text-gray-500 hover:text-gray-900 hover:bg-black/5"
            >
              {label}
            </a>
          ))}
          ...
          {user ? (
            <div className="relative" ref={dropRef}>
              <button
                ...
                className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-full border border-black/10 bg-black/4 hover:bg-black/8 hover:border-black/15 transition-all duration-200"
              >
                <img
                  src={user.avatar}
                  alt={user.name}
                  className="h-7 w-7 rounded-full object-cover ring-1 ring-[#6c47ff]/50"
                />
                <span className="hidden sm:block text-[13px] font-medium text-gray-700 max-w-[120px] truncate">
                  {user.name.split(" ")[0]}
                </span>
                <svg
                  className={`h-3 w-3 text-gray-400 transition-transform duration-200 ${avatarOpen ? "rotate-180" : ""}`}
                  viewBox="0 0 12 12"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
              ...
              <AnimatePresence>
                {avatarOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-[calc(100%+8px)] w-56 rounded-2xl border border-black/8 bg-white shadow-[0_16px_48px_-8px_rgba(0,0,0,0.18)] z-50 overflow-hidden"
                  >
                    ...
                    <div className="p-1.5">
                      <button
                        ...
                        className="w-full text-left px-3 py-2 rounded-xl text-[13px] text-gray-500 hover:text-red-500 hover:bg-red-50 transition-all duration-150 flex items-center gap-2.5 group"
                      >
                        <svg
                          className="h-3.5 w-3.5 text-gray-400 group-hover:text-red-400 transition-colors"
                          viewBox="0 0 16 16"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="1.5"
            ...
            <>
              <MagneticButton
                ...
                className="hidden sm:inline-flex items-center gap-2 text-[13px] px-4 py-2 rounded-full border border-black/12 text-gray-600 hover:text-gray-900 hover:border-black/20 hover:bg-black/4 font-medium transition-all duration-200"
              >
                Sign in
              </MagneticButton>
              <MagneticButton
                ...
                className="inline-flex items-center gap-2 text-[13px] px-4 py-2 rounded-full font-semibold text-white transition-all duration-200 shadow-[0_0_20px_-4px_rgba(108,71,255,0.55)] hover:shadow-[0_0_26px_-2px_rgba(108,71,255,0.7)] hover:scale-[1.02]"
                style={{
...
function Hero() {
  return (
    <section className="relative hero-canvas overflow-hidden">
      ...
      <Spotlight className="relative z-10">
        <div className="relative max-w-6xl mx-auto px-5 pt-20 pb-28 md:pt-28 md:pb-36 flex flex-col items-center text-center">
          <div className="inline-flex items-center gap-2 text-[12px] text-[var(--ink)] px-3 py-1.5 rounded-full border border-[var(--border-active)] bg-white/90 backdrop-blur mb-8 shadow-sm animate-fade-in">
            <motion.span
          ...
          <h1 className="text-display text-hero max-w-4xl animate-fade-up">
            Say goodbye to your{" "}
            <span className="italic relative inline-block text-gradient-animated">
              human
          ...
          <div className="mt-14 w-full max-w-4xl animate-fade-up perspective-[1600px]" style={{ animationDelay: "160ms" }}>
            <TopicInput />
          </div>

          <div
            className="mt-10 flex items-center gap-6 text-[12.5px] text-[var(--text-secondary)] animate-fade-up"
  ...
  return (
    <div className="border-y border-[var(--border)] bg-white/50 backdrop-blur py-5 overflow-hidden">
      <div className="flex w-max marquee-track gap-12">
        {doubled.map((l, i) => (
          <div key={i} className="flex items-center gap-12 text-[13px] tracking-[0.18em] font-medium">
            <span className="marquee-glow">{l}</span>
...
function EditorMock() {
  return (
    <section id="editor" className="relative ink-section py-24 md:py-32 overflow-hidden">
      ...
      <motion.div
        className="absolute top-0 left-1/2 -translate-x-1/2 h-[300px] w-[800px] bg-[var(--accent)]/20 blur-[120px] rounded-full pointer-events-none"
      ...
      <div className="relative max-w-6xl mx-auto px-5">
        ...
        <TiltCard className="rounded-3xl border border-white/10 bg-[#0f0f15] shadow-[0_60px_120px_-40px_rgba(108,71,255,0.4)] overflow-hidden">
          {/* top toolbar */}
          ...
          <div className="relative p-5 space-y-2 bg-[#0d0d12]">
            ...
            {/* playhead */}
            <div className="absolute top-4 bottom-4 w-px bg-emerald-400 playhead pointer-events-none shadow-[0_0_20px_rgba(52,211,153,1)]">
              <div className="absolute -top-1 -left-[5px] h-3 w-3 rotate-45 bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.9)]" />
            </div>
          </div>
        </TiltCard>

        <Reveal delay={0.15} className="mt-8 text-center text-[14px] text-white/50 max-w-xl mx-auto">
      ...
      <div className="flex gap-[2px] items-center flex-1 h-6">
        {Array.from({ length: 80 }).map((_, i) => {
          ...
          return (
            <span
              key={i}
              className="w-[3px] rounded-full bg-amber-300/70 bgm-bar"
    ...
    <section id="features" className="py-24 md:py-32">
      <Reveal className="max-w-3xl mx-auto px-5 text-center">
        ...
        <h2 className="text-display text-h1 mb-5">
          Jam-packed with tools in every <span className="italic text-gradient-animated">step.</span>
        </h2>
...
function FeatureScript() {
  return (
    <section className="max-w-7xl mx-auto px-5 pb-20">
      <Reveal>
        <TiltCard intensity={6} className="grid md:grid-cols-2 gap-6 rounded-3xl border border-[var(--border)] bg-gradient-to-br from-white to-[#f5f3ff] p-8 md:p-12 overflow-hidden feature-3d">
          ...
          <div className="relative">
            ...
            <motion.div
              ...
              className="absolute -top-3 -right-3 px-3 py-1 rounded-lg bg-white border border-[var(--border)] text-[11px] font-medium shadow-md"
            >
              ✦ AI suggested
            </motion.div>
          </div>
...
function FeatureTeamAndStyle() {
  return (
    <Stagger className="max-w-7xl mx-auto px-5 pb-20 grid md:grid-cols-2 gap-6">
      <StaggerItem>
        <TiltCard intensity={8} className="rounded-3xl border border-[var(--border)] bg-white p-8 md:p-10 relative overflow-hidden feature-3d">
      ...
      <StaggerItem>
        <TiltCard intensity={8} className="rounded-3xl border border-[var(--border)] bg-gradient-to-br from-[#0f0f15] to-[#1c1635] text-white p-8 md:p-10 relative overflow-hidden feature-3d">
...
function FeatureFootage() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-5 pb-24">
      <Reveal>
        <TiltCard intensity={5} className="grid md:grid-cols-[1fr_1.2fr] gap-6 rounded-3xl border border-[var(--border)] bg-white p-8 md:p-12 overflow-hidden feature-3d">
          <div className="flex flex-col justify-center max-w-md">
            <h3 className="text-display text-[36px] leading-[1.05] tracking-tight mb-4">
              HD footage, <span className="italic text-gradient-animated">curated</span> for every scene.
            </h3>
          ...
          <div className="relative bg-gradient-to-br from-[#1a1330] to-[#0d0d15] rounded-2xl p-5 overflow-hidden min-h-[360px]">
            ...
            <motion.div
              animate={{ y: [0, -4, 0] }}
              transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }}
              className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-2.5 rounded-full bg-gradient-to-r from-[var(--accent)] to-[#a78bfa] text-white text-[13px] font-medium shadow-[0_12px_40px_-8px_var(--accent-glow)]"
            >
            ...
            <StaggerItem key={t.name}>
              <motion.div
                ...
                className={`relative rounded-3xl p-8 border overflow-hidden ${
                  t.highlight
                    ? "bg-[var(--ink)] text-white border-[var(--ink)] shadow-[0_40px_100px_-30px_var(--accent-glow)] pricing-shine"
                    : "bg-white border-[var(--border)] hover:border-[var(--border-active)]"
                }`}
              >
                {t.highlight && (
    ...
    <section className="px-5 pb-12">
      <Reveal>
        <div className="relative max-w-7xl mx-auto rounded-3xl bg-gradient-to-br from-[#6c47ff] via-[#8b6aff] to-[#a78bfa] overflow-hidden p-12 md:p-20 text-center text-white">
          ...
          <div className="absolute -top-32 -left-32 h-72 w-72 bg-white/20 blur-[80px] rounded-full cta-mesh" />
          <div className="absolute -bottom-32 -right-32 h-72 w-72 bg-pink-300/30 blur-[80px] rounded-full cta-mesh" style={{ animationDelay: "3s" }} />
          ...
          <div className="relative flex flex-wrap justify-center gap-3">
            <MagneticButton className="px-5 py-3 rounded-xl bg-white text-[var(--ink)] font-medium hover:bg-white/90 transition-colors flex items-center gap-2">
              <Play className="h-3.5 w-3.5 fill-current" /> Watch demo
            </MagneticButton>
            <MagneticButton className="px-5 py-3 rounded-xl bg-[var(--ink)] text-white font-medium hover:bg-black transition-colors flex items-center gap-2 shadow-[0_16px_40px_-12px_rgba(0,0,0,0.5)]">
